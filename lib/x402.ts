...
const DEMO_MODE = !process.env.AGENT_PRIVATE_KEY || !process.env.GOAT_RPC_URL;

export async function payAgent(toAddress: string, amountUsdt: string): Promise<string> {
  if (DEMO_MODE) {
    // Return a fake txHash in demo mode
    const fakeTx = "0x" + Math.random().toString(16).slice(2).padEnd(64, "0");
    console.log(`[DEMO] Simulated payment of ${amountUsdt} USDT to ${toAddress}: ${fakeTx}`);
    return fakeTx;
  }
...
export async function verifyPayment(txHash: string, expectedTo: string, expectedAmount: string): Promise<boolean> {
  if (DEMO_MODE) {
    console.log(`[DEMO] Skipping payment verification for ${txHash}`);
    return true;
  }
  const provider = new ethers.JsonRpcProvider(process.env.GOAT_RPC_URL);
  ...
  for (const log of receipt.logs) {
    try {
      ...
      if (to === expectedTo.toLowerCase() && value >= expected) return true;
    } catch {}
  }
  return false;
}
...
export function proofOfPayment(txHash: string, from: string, to: string) {
  return {
    proofOfPayment: { fromAddress: from, toAddress: to, chainId: 48816, txHash },
  };
}
