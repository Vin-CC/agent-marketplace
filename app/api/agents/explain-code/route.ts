...
export async function POST(req: NextRequest) {
  ...
  const result = await callClaude(
    "You are a code explanation agent. Explain the given code in plain English, step by step. Make it understandable for a non-technical person.",
    code
  );
  return NextResponse.json({ result, agent: "Code Explainer", proofOfPayment: { txHash, fromAddress, toAddress: AGENT_WALLET, chainId: 48816 } });
}
