...
const AGENT_WALLET = process.env.AGENT_EXPLAINER_WALLET || process.env.AGENT_PRIVATE_KEY!;
const PRICE = "0.10";
...
export async function POST(req: NextRequest) {
  ...
  const response = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      model: "gpt-4o-mini",
      messages: [
        { role: "system", content: "You are a code explanation agent. Explain the given code in plain English, step by step. Make it understandable for a non-technical person." },
